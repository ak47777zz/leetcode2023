...
public class 相交链表160 {
    ...
    public class Solution {
        ...
        public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
            // 解法1：使用hash存储第一个链表元素 空间复杂度O(m) m是链表A的长度
            // 解法2：双指针，A遍历完接B，B遍历完接A，值相等的点就是交点
            ListNode A = headA;
            ListNode B = headB;
            // 并不走到最后一个有数的节点就走另一条了，最后有个null也要走一下，所以不想交的两条也是有一个共同的节点位节点null
            while (A != B) {
